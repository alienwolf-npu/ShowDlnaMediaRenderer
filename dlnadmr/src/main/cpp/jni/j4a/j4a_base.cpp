...
int J4A_ThrowException(JNIEnv* env, const char* class_sign, const char* msg)
{
    ...
    ret = 0;
fail:
    J4A_DeleteLocalRef__p(env, (jobject *)&exceptionClass);
    return ret;
}
...
{
    jclass obj_global = (jclass)env->NewGlobalRef(obj);
...
fail:
    J4A_DeleteLocalRef__p(env, (jobject *)&clazz);
    return clazz_global;
}
...
jbyteArray J4A_NewByteArray__asGlobalRef__catchAll(JNIEnv *env, jsize capacity)
{
    ...
    jbyteArray global = (jbyteArray) (env->NewGlobalRef(local));
    J4A_DeleteLocalRef__p(env, (jobject *)&local);
    return global;
}
