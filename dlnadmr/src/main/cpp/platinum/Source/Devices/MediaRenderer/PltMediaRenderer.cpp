...
PLT_MediaRenderer::PLT_MediaRenderer(const char*  friendly_name, 
    ...
    m_Delegate(NULL)
{
    m_ModelDescription = "Show AV Media Renderer Device";
    m_ModelName        = "AV Renderer Device";
    m_ModelURL         = "https://www.show.com/open";
    m_DlnaDoc          = "DMR-1.0";
}
                            ...
                            const PLT_HttpRequestContext& context)
{
    ...
    NPT_LOG_INFO_1("onAction: %s", name.GetChars());

	int isCurrentDmc = 1;
    const NPT_String* agent  = context.GetRequest().GetHeaders().GetHeaderValue(NPT_HTTP_HEADER_USER_AGENT);
    const NPT_String &ip = context.GetRemoteAddress().GetIpAddress().ToString();
    if (!(ip.IsEmpty()) && strlen(m_DmcIP) > 0 && ip.Compare(m_DmcIP) != 0) {
        NPT_LOG_INFO("not current connected device, return");
        isCurrentDmc = 0;
    }
    if (agent != NULL && !(agent->IsEmpty()) && strlen(m_DmcUserAgent) > 0 && agent->Compare(m_DmcUserAgent) != 0 ) {
        NPT_LOG_INFO("same ip, different ua-0, return");
        isCurrentDmc = 0;
    }

    if ((agent == NULL || agent->IsEmpty()) && strcmp(m_DmcUserAgent, "none") != 0) {
        NPT_LOG_INFO("same ip, different ua-1, return");
        isCurrentDmc = 0;
    }

    if (agent != NULL && !agent->IsEmpty() && strcmp(m_DmcUserAgent, "none") == 0) {
        NPT_LOG_INFO("same ip, different ua-2, return");
        isCurrentDmc = 0;
    }

    if (!isCurrentDmc) {
        if (name.Compare("Next", true) == 0 || name.Compare("Pause", true) == 0
            || name.Compare("Play", true) == 0 || name.Compare("Previous", true) == 0
            || name.Compare("Seek", true) == 0 || name.Compare("Stop", true) == 0
            || name.Compare("SetVolume", true) == 0 || name.Compare("SetMute", true) == 0) {
            return NPT_FAILURE;
        }

        if (name.Compare("GetPositionInfo", true) == 0) {
            NPT_LOG_INFO("GetPostionInfo return default for non-current-dmc");
            ...
            action->SetArgumentValue("RelCount", "2147483647");
            action->SetArgumentValue("AbsCount", "2147483647");
            return NPT_SUCCESS;
        }

        if (name.Compare("GetTransportInfo", true) == 0) {
            NPT_LOG_INFO("GetTransportInfo return default for non-current-dmc");
            action->SetArgumentValue("CurrentTransportState", "STOPPED");
    ...
    if (name.Compare("SetAVTransportURI", true) == 0) {
        const NPT_String* agent  = context.GetRequest().GetHeaders().GetHeaderValue(NPT_HTTP_HEADER_USER_AGENT);
        memset(m_DmcUserAgent, 0x0, sizeof(m_DmcUserAgent));
        memset(m_DmcIP, 0x0, sizeof(m_DmcIP));
        strncpy(m_DmcUserAgent, (agent && !agent->IsEmpty()) ? agent->GetChars() : "none", sizeof(m_DmcUserAgent));
        const NPT_String &ip = context.GetRemoteAddress().GetIpAddress().ToString();
        strncpy(m_DmcIP, !ip.IsEmpty() ? ip.GetChars() : "0.0.0.0", sizeof(m_DmcIP));
        NPT_LOG_INFO_2("User-Agent: %s IP: %s", m_DmcUserAgent, m_DmcIP);
